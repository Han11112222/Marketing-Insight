# app.py — Gas Sales Analytics (Parquet + Excel ready)
# - 탭0: 연도×용도 누적 막대 (랜딩)
# - 탭1: 월/분기/반기/연간 집계
# - 탭2: 산업용 집중분석 — 업종×기간 히트맵 + 업종/기간 선택 → 고객 Top-N
...
st.sidebar.caption("A: 월별 총괄(주택/산업 합산), B: 산업용 상세(고객/업종) — CSV/Parquet 권장(B는 Parquet 이미 준비됨)")

# A) 월별 총괄: 업로드가 없으면 repo의 '상품별판매량.xlsx' 자동 사용
...
if up_overall is not None:
    ...
    if used_overall.lower().endswith(".parquet"):
        overall_raw = pd.read_parquet(up_overall)
    ...
    # 저장소 파일 자동 사용
    if os.path.exists("상품별판매량.parquet"):
        overall_raw = pd.read_parquet("상품별판매량.parquet")
        used_overall = "상품별판매량.parquet"
        st.sidebar.info("A 자동 사용: **상품별판매량.parquet**")
    elif os.path.exists("상품별판매량.xlsx"):
        overall_raw = read_excel_any("상품별판매량.xlsx")
        used_overall = "상품별판매량.xlsx"
        st.sidebar.info("A 자동 사용: **상품별판매량.xlsx**")
    else:
        st.info("A 파일이 필요합니다. CSV 또는 Parquet로 업로드/저장해 주세요.")
        st.stop()

colsA = overall_raw.columns.astype(str).tolist()
st.sidebar.header("② A(월별 총괄) 컬럼 매핑")

def _pickA(keys, fallback_idx=0):
    for k in keys:
        for c in colsA:
            if k in c:
                return c
    return colsA[fallback_idx]
...
# B) 산업용 상세: 업로드 없으면 repo의 Parquet 자동 병합
st.sidebar.header("③ B(산업용 상세) 파일")
up_indetail = st.sidebar.file_uploader("B) 산업용 상세 — CSV/XLSX/Parquet 여러 개", type=["csv", "xlsx", "xls", "parquet"], accept_multiple_files=True)
used_inds = []
if up_indetail:
    frames = []
    ...
    pats = ["가정용외_*.parquet"]  # 너가 올려둔 파일명 규칙 사용
    files = list_files(pats)
    if files:
...
if len(colsB) > 0:
    b_date = st.sidebar.selectbox("날짜(월/청구년월)", colsB, index=(colsB.index(_pickB(["청구년월", "사용월", "년월", "날짜"])) if _pickB(["청구년월", "사용월", "년월", "날짜"]) in colsB else 0))
    ...
    b_amt  = st.sidebar.selectbox("사용량 열", colsB, index=(colsB.index(_pickB(["사용량(m3", "m3사용량", "사용량", "수량", "NM3", "Nm3", "MJ"])) if _pickB(["사용량(m3", "m3사용량", "사용량", "수량", "NM3", "Nm3", "MJ"]) in colsB else 0))
else:
    b_date = b_use = b_ind = b_cus = b_amt = None

def parse_month(x):
    s = str(x)
    for fmt in ["%Y-%m", "%Y/%m", "%Y%m", "%Y.%m", "%y-%b", "%Y-%m-%d", "%Y/%m/%d"]:
        try:
            # "%y-%b" 같은 25-Sep 케이스 커버
...
date_min = pd.to_datetime(min(overall["날짜"].min(), indetail["날짜"].min())) if len(indetail) > 0 else pd.to_datetime(overall["날짜"].min())
date_max = pd.to_datetime(max(overall["날짜"].max(), indetail["날짜"].max())) if len(indetail) > 0 else pd.to_datetime(overall["날짜"].max())

d1, d2 = st.sidebar.date_input("기간", [date_min, date_max])
d1 = pd.to_datetime(d1); d2 = pd.to_datetime(d2)
...
with tab0:
    ...
    landing = overall[(overall["날짜"] >= d1) & (overall["날짜"] <= d2)].copy()
    ...
    st.dataframe(annual.set_index("연도").style.format("{:,.0f}"), use_container_width=True, height=280)

# ===== 탭1: 집계 =====
with tab1:
    st.subheader("집계 — 월/분기/반기/연간 (주택용 / 산업용)")
    gran = st.radio("집계 단위", ["월", "분기", "반기", "연간"], horizontal=True, key="granularity")
    A = overall[(overall["날짜"] >= d1) & (overall["날짜"] <= d2)].copy()
    A["Period"] = as_period_key(A["날짜"], gran)
    sum_tbl = A.groupby("Period", as_index=False)[["주택용", "산업용"]].sum().sort_values("Period")

    left, right = st.columns([2, 3])
    with left:
        st.dataframe(sum_tbl.style.format({"주택용": "{:,.0f}", "산업용": "{:,.0f}"}), use_container_width=True, height=360)
    with right:
        fig = go.Figure()
...
# ===== 탭2: 산업용 집중분석 =====
with tab2:
    st.subheading = st.subheader("산업용 집중분석 — 업종 히트맵 → 고객 Top-N")
    if len(indetail) == 0:
        st.info("산업용 상세 파일(B)이 없어 히트맵을 표시할 수 없어.")
    else:
        gran_focus = st.radio("기간 단위", ["월", "분기", "반기", "연간"], horizontal=True, key="gran_focus")
        B = indetail[(indetail["날짜"] >= d1) & (indetail["날짜"] <= d2)].copy()
        # 파일에 다른 용도가 섞여 있는 경우 산업만 필터
        if "용도" in B.columns:
            B = B[B["용도"].str.contains("산업", na=False)]

        if len(B) == 0:
            st.info("해당 기간/필터에서 산업용 데이터가 없습니다.")
        else:
            ...
            # 셀 안에 값 + 업종 라벨을 함께 표시(값은 줄바꿈)
            text = [[f"{y}<br>{z:,.0f}" for z in row] for y, row in zip(Y, Z)]
            ...
            # ② 선택 컴포넌트(클릭 모듈 없이도 분석 가능)
            sel_ind   = st.selectbox("업종 선택(Top-N 보기)", Y)
