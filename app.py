# app.py — Gas Sales Analytics (Fast+Robust)
# 탭3: 업종×기간 히트맵 → 셀 클릭(가능시) / 기간·업종 선택 폴백
...
def yoy_compare(df, key_cols, value_col, period_col, gran: str):
    lag = {"월":12,"분기":4,"반기":2,"연간":1}.get(gran,12)
    ...
    b = (cur.rename(columns={period_col:"_prev"})
           ...
           .sum().rename(columns={value_col:"전년동기"}))
...
# A) 월별 총괄
up_overall = st.sidebar.file_uploader("A) 월별 총괄(.parquet/.csv/.xlsx)",
...
used_inds=[]
if up_indetail:
    frames=[read_any(f, f.name) for f in up_indetail]
    ...
    if files:
        frames=[read_any(p, p) for p in files]
        indetail_raw = pd.concat(frames, ignore_index=True) if frames else pd.DataFrame()
...
c_date   = st.sidebar.selectbox("날짜(월)", colsA, index=colsA.index(pickA(["날짜","월","Date"])) if pickA(["날짜","월","Date"]) in colsA else 0)
...
if len(colsB)>0:
    b_date = st.sidebar.selectbox("날짜(월)", colsB, index=(colsB.index(pickB(["청구년월","사용월","년월","월"])) if pickB(["청구년월","사용월","년월","월"]) in colsB else 0))
    ...
    b_amt  = st.sidebar.selectbox("사용량 열", colsB, index=(colsB.index(pickB(["사용량","사용량(m3","m3사용량","NM3","Nm3","수량","MJ"])) if pickB(["사용량","사용량(m3","m3사용량","NM3","Nm3","수량","MJ"]) in colsB else 0))
    ...
    indetail["사용량"] = pd.to_numeric(indetail[b_amt].astype(str).str.replace(",","").str.replace(" ",""),
...
with tab2:
    ...
    # 업종×기간 피벗(캐시 없음: 구간/필터가 바뀌므로 바로 계산)
    ...
    if HAS_PLOTLY_EVENTS:
        ...
        sel_from_click = False

    # 폴백 선택 UI(클릭이 안 되었거나 모듈이 없는 경우)
    if not sel_from_click:
        cc1, cc2, _ = st.columns([1.5,1.5,2])
        sel_ind = cc1.selectbox("업종 선택", Y, index=0)
        sel_period = cc2.selectbox("기간 선택", X, index=len(X)-1)
...
with st.expander("🔎 분석에 사용된 원천 파일"):
    if 'used_overall' in locals() and used_overall: st.write(f"A(월별 총괄): **{used_overall}**")
